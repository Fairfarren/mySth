...
export const setWxComfig = (state, data) => {
  state.weChatConfig = data
}

export const wxMenuShare = (state) => {
  const theClass =
    state.CLASS.name
      ? state.CLASS
      : {
        name: '汝课堂',
        link: 'http://www.ruketang.com/wechat',
  ...
  wx.config({
    debug: false,
    appId: state.weChatConfig.appId,
    timestamp: state.weChatConfig.timestamp,
    nonceStr: state.weChatConfig.noncestr,
    signature: state.weChatConfig.sign,
