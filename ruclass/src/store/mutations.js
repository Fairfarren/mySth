...
export const WATCH_DETAILED_SHOW_TRUE = (state) => {
    ...
    state.DETAILED_SHOW_MARGIN = '88px';
}
...
export const SET_USER_INFO = (state, user) => {
    state.USER.name = user.username || 'hello';
...
export const POPUP_PAY_CLASS_STH = (state, data) => {
    ...
    //     price: data.price
    // }
    state.POPUP_PAY_CLASS.name = data.name;
    state.POPUP_PAY_CLASS.publisher = data.publisher;
    state.POPUP_PAY_CLASS.lecturer = data.lecturer;
    state.POPUP_PAY_CLASS.time = data.time;
    state.POPUP_PAY_CLASS.price = data.price;
}
//站内支付，储存课程信息
